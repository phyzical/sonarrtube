...
class FileHandler {
  ...
  getFilesToProcess(): Record<string, unknown> {
    ...
    const filesForProcessing = directories.reduce(
      (seriesAcc: Record<string, unknown>, series: string): Record<string, unknown> => {
        ...
        const seasonAccumulator = (seasonAcc: Record<string, unknown>, season: string): Record<string, unknown> => {
          ...
          const episodeAccumulator = (key: string): Episode => {
            const informationFile = files.find((file) => file.includes(key) && file.includes('.json'));
            let thumbnailFile = files.find(
              (file) => file.includes(key) && (file.includes('-screen.jpg') || file.includes('-thumb.jpg'))
            );
